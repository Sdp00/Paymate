...
export const seedDatabase = mutation({
  ...
  handler: async (ctx) => {
    ...
    const groups = await createdGroups(ctx, users);

    // Step 3: Created 1-on-1 expenses
    const oneOnOneExpenses = await createdOneOnOneExpenses(ctx, users);

    // Step 4: Created group expenses
    const groupExpenses = await createdGroupExpenses(ctx, users, groups);

    // Step 5: Created settlements
    const settlements = await createdSettlements(
      ctx,
      users,
      groups,
      oneOnOneExpenses,
...
// Helper to created groups
async function createdGroups(ctx, users) {
  const now = Date.now();
...
// Helper to created one-on-one expenses
async function createdOneOnOneExpenses(ctx, users) {
  const now = Date.now();
...
// Helper to created group expenses
async function createdGroupExpenses(ctx, users, groups) {
  const now = Date.now();
  const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
  const twoWeeksAgo = now - 14 * 24 * 60 * 60 * 1000;
...
// Helper to created settlements
async function createdSettlements(
  ctx,
  users,
  groups,
  oneOnOneExpenses,
